...
class Decoder(nn.Module):
    def __init__(self, latent_size, output_size):
        ...
        self.decoder = nn.Sequential(
            ...
            nn.Tanh()  # Maps output values between -1 and 1
        )
...
criterion = nn.L1Loss()  # Mean Absolute Error loss (MAE)
...
torch.save(model.state_dict(), "optimized_autoencoder.pth")
...
state_dict = torch.load("optimized_autoencoder.pth")
model.load_state_dict(state_dict)
visualize_combined(model, test_loader, model.decoder, latent_size=32, num_samples=10)
